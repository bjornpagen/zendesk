...
const findMatchingProblem = inngest.createFunction(
	{ id: "problems-find-matching" },
	{ event: "problems/find-matching" },
	async ({ event, step }) => {
		const { threadId } = event.data
		...
		const completion = await step.run("openai-classify", () =>
...
const createProblem = inngest.createFunction(
	{ id: "problems-create-new" },
	{ event: "problems/create-new" },
	async ({ event, step }) => {
		const { threadId } = event.data
		...
		const completion = await step.run("openai-create", () =>
			openai.beta.chat.completions.parse({
				model: "gpt-4o",
				messages: [
					{
...
${thread.messages.map((m) => `[${m.type}] ${m.content}`).join("\n\n")}`
		...
		const inserted = await step.run("db-insert", () =>
			db
				.insert(schema.problems)
				.values({
					title: result.title,
